...
$(function(){
	...
	$(document).on("mouseenter","li[ectype='floor_cat_content']",function(){
		...
		var visualhome = $(this).data("visualhome");//可视化模板标识
			...
			if(data.error == 0){
				pb({
					...
					onOk:function(){
						...
						Ajax.call('ajax_dialog.php?act=gotoInvoice','inv_content='+encodeURIComponent(inv_content)+'&invoice_id='+invoice_val +'&store_id='+store_id +'&invoice_type='+invoice_type + '&shipping_id=' + $.toJSON(arr),gotoInvoiceResponse, 'POST', 'JSON');
	...
	function invoice(){
		...
		$("*[ectype='nextStep']").on("click",function(){
			var type = $(this).data("type"),				
				...
				msg.consignee_province = frm.find("input[name='consignee_province']").val();
