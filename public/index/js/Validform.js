$.Validform = function(a,b,c){
	...
	nameValid = function(val,error){
		if(val == ''){
			...
			error.html(i_error + input_username);
			return false;
		}else if(!/^[a-zA-Z0-9\_\e4e00-\u9fa5]{2,20}$/.test(val)){
			//验证用户名长度2-20字符之间
			error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
			error.removeClass().addClass("error");
			error.html(i_error+password_lenght);
			return false;
			...
			Ajax.call('user.php?act=is_user', 'username=' + val, function(data){
				if (data.result == "false"){ 
					...
					error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
					error.removeClass().addClass("error");
					error.html(i_error+username_are);
					return_user = 1;
					return false;
				}
			} , 'GET', 'JSON', true, true);	
	...
	passwordValid = function(val,error,state){
		...
		if(state == 0){
			if(val == ''){
				error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
				error.removeClass().addClass("error");
        		error.html(i_error + password_null);
        ...
        if(m>0){
            ...
            if( m == 1 && val.length >5){
				error.parent().prev().removeClass("form-item-error").addClass("form-item-valid");
			    error.html("<i class='i-pwd-weak'></i>"+login_password_packup_one);
				error.removeClass().addClass("strength r_strength");
            }
            if(m == 2){
				error.parent().prev().removeClass("form-item-error").addClass("form-item-valid");
				error.html("<i class='i-pwd-medium'></i>"+login_password_packup_two);
				error.removeClass().addClass("strength z_strength");
            }   
            if(m == 3){
				error.parent().prev().removeClass("form-item-error").addClass("form-item-valid");
				error.html("<i class='i-pwd-strong'></i>"+login_password_packup_three);
	...
	confirmPasswordValid = function(val,error){
		...
		if(val == ''){
			//验证确认密码是否为空
			error.html(i_error + confirm_password);
			return false;
		}else if(val != pwd){
			//验证确认密码和密码是否一致
			error.html(i_error+Dont_agree_password);
			return_pwd = 1;
			return false;
		}else{
			error.parent().prev().removeClass("form-item-error").addClass("form-item-valid");
	...
	phoneValid = function(val,error){
		if(val == ''){
			...
			error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
			error.removeClass().addClass("error");
			error.html(i_error + login_phone_packup_one);
			return false;
		}else if(!(Utils.isPhone(val))){
			//验证手机格式
			error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
			error.removeClass().addClass("error");
			error.html(i_error+Mobile_error);
			return false;
		}else{
			Ajax.call('user.php?act=check_phone', 'mobile_phone=' + val, function(result){
				if ( result.replace(/\r\n/g,'') == 'ok' ){
					...
					error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
					error.removeClass().addClass("error");
					error.html(i_error+exist_phone);
					return_mobile = 1;
					return false;
				}
			} , 'GET', 'TEXT', true, true);	
	...
	codeValid = function(val,error){
		if(val == ''){
			...
			error.removeClass().addClass("error");
			//验证验证码是否为空
			error.html(i_error + null_captcha_login);
			return false;
		}else{
			Ajax.call('user.php?act=is_register_captcha', 'captcha=' + val, function(res){
				if (res.result == "false"){
					...
					error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
					error.removeClass().addClass("error");
					error.html(i_error+error_email_login);
					return_captcha = 1;
					return false;
				}else{
					error.parent().prev().removeClass("form-item-error").addClass("form-item-valid");
	...
	telcodeValid = function(val,error){
		if(val == ''){
			...
			error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
			error.removeClass().addClass("error");
			error.html(i_error + null_captcha_login_phone);
			return false;
		}else{
			Ajax.call('user.php?act=is_mobile_code', 'mobile_code=' + val, function(data){
				if (data.result == "ok"){ 
					...
					error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
					error.removeClass().addClass("error");
					error.html(i_error+error_captcha_login_phone);
